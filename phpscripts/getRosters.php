...
$sql = "SELECT CONCAT(UPPER(profiles.first_name), ' ', UPPER(profiles.second_name), ' ', UPPER(profiles.first_lastname), ' ', UPPER(profiles.second_lastname)) AS `name`, 
        ...
        h.start AS `sun_start`, h.end AS `sun_end`, rosters.week_value, COALESCE(`cnt`.`count`,1) AS `count`, idemployees, id_account From employees
        ...
        WHERE rosters.id_period IS NULL OR rosters.id_period = $str AND employees.active = '1';";
