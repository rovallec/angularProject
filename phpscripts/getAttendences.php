...
if(explode(" ", $date)[0] === "<="){
	...
	FROM `hires`
		...
		LEFT JOIN (SELECT * FROM `attendences` WHERE `date` $date) AS `att` ON `att`.`id_employee` = `employees`.`idemployees`) AS `attend` WHERE `idprofiles` = $id ORDER BY `date` ASC;";
}else{
	if($id == 'NULL'){
		$id_emp = explode(";", $date);
		$dt = $id_emp[0];
		$exp_id = $id_emp[1];
		$sql = "SELECT * FROM `attendences` ORDER BY `date` DESC  WHERE `date` = '$dt' AND `id_employee` = '$exp_id'";
		}else{
