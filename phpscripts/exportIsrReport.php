...
if($result = mysqli_query($con,$sql)){
    while($row = mysqli_fetch_assoc($result)){
        ...
        $isr[4] = number_format((($row['productivity'] - 250) * (12 - date("m",strtotime($end))) + (($row['productivity'] - 250) * $monthly_mult) + $row['print_productivity'] + $row['bonuses'] + (($row['productivity']) * ($b_days/365)) + (($row['productivity']) * ($a_days/365))),2);
        $isr[5] = '0';
        $isr[6] = '0';
//////////////////////////////////////////////////AGUINALDO//////////////////////////////////////////////////////////////
