<?php
    ...
    SUM(coalesce(`adj`.`amnt`, 0))  AS `adjustments`, SUM(coalesce(`hld`.`amnt`,0)) AS `hol`, AVG(employees.productivity_payment) AS `prod_pay`
    FROM employees
    ...
    WHERE active = 1 AND length(profiles.nit) > 0 AND `cmp_base` > 0 GROUP BY idemployees;";
...
if($result = mysqli_query($con,$sql)){
    while($row = mysqli_fetch_assoc($result)){
        ...
        $isr[4] = number_format((($row['prod_pay']) * (12 - date('m',strtotime($end))) + (($row['prod_pay']) * $monthly_mult) + $row['print_productivity'] + $row['bonuses'] + (($row['prod_pay'] + 250) * ($b_days/365)) + (($row['prod_pay'] + 250) * ($a_days/365)) + ($row['adjustments'])),2);
