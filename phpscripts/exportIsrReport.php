...
coalesce(`real_base`,0) AS `print_base`, coalesce(`real_productivity`,0) AS `print_productivity`, SUM(coalesce(`b_decreto`.`b_amt`,0)) AS `decreto_acumulado` FROM employees
...
if($result = mysqli_query($con,$sql)){
    while($row = mysqli_fetch_assoc($result)){
        ...
        $isr[4] = number_format((($row['productivity'] - 250) * (12 - date("m",strtotime($end))) + (($row['productivity'] - 250) * $monthly_mult) + $row['print_productivity'] + $row['bonuses']),2);
        $isr[5] = '0';
        $isr[6] = '0';
//////////////////////////////////////////////////AGUINALDO//////////////////////////////////////////////////////////////
