...
$result = [];
$sql = "SELECT users.user_name, asset_movements.notes, asset_movements.date, assets.idassets, first_name, second_name, first_lastname, second_lastname, nearsol_id, dpi, accounts.name AS `account`,
		...
		WHERE idemployees = $id_employee AND asset_movements.status = '1';";
...
if($result2 = mysqli_query($con, $sql))
{
	...
	{
		$result[$i]['idassets'] = $row['idassets'];
