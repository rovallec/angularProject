...
if($netsuitclient <= 6){
    ...
    ROUND(IF(employees.job_type = 1, 0, ROUND(employees.base_payment/2,2)),2) AS `base_pay`,
    ROUND(IF(employees.job_type = 1,IF(employees.cost_type IS NULL, 0, ((payments.productivity_complete - (employees.max_cost - payments.base_complete - 250))/2)), ROUND(employees.productivity_payment/2,2)),2) AS `productivity_pay`,
...
employees.client_id,
CONCAT(profiles.first_name, ' ', profiles.second_name, ' ', profiles.first_lastname, ' ', profiles.second_lastname) AS `employee name`,
ROUND(IF(employees.job_type IS NULL, 0, ROUND(employees.base_payment/2,2)),2) AS `base_pay`,
ROUND(IF(employees.job_type IS NULL,IF(employees.cost_type IS NULL, 0, ((payments.productivity_complete - (employees.max_cost - payments.base_complete - 250))/2)), ROUND(employees.productivity_payment/2,2)),2) AS `productivity_pay`,
