...
$sql = "SELECT `profiles`.`first_name`, `profiles`.`second_name`, `profiles`.`first_lastname`, `profiles`.`second_lastname`, `profiles`.`tittle`, `profiles`.`day_of_birth`, `hires`.`nearsol_id`, `employees`.`hiring_date`, `employees`.`job`, `proc`.`motive`, `proc`.`kind`, `proc`.`reason`, `proc`.`rehireable`, `proc`.`valid_from`, `proc`.`date`, `recriuter`.`user_name` AS `reciutment`, `ops`.`value` AS `operations`, `usrs`.`user_name` AS `sup`, `exp`.`company` AS `experience`, `accounts`.`name` AS `LOB`, `proc`.`nearsol_experience`, `proc`.`supervisor_experience`, `education_details`.`current_level` FROM `profiles` LEFT JOIN `hires` ON `hires`.`id_profile` = `profiles`.`idprofiles` LEFT JOIN `employees` ON `employees`.`id_hire` = `hires`.`idhires` LEFT JOIN (SELECT * FROM hr_processes LEFT JOIN `terminations` ON `terminations`.`id_process` = `hr_processes`.`idhr_processes`WHERE `id_type` = 8) AS `proc` ON `proc`.`id_employee` = `employees`.`idemployees` LEFT JOIN (SELECT `user_name`, `processes`.`id_profile` FROM `processes` LEFT JOIN `users` ON `users`.`idUser` = `processes`.`id_user` WHERE `name` = 'New Hire') 
AS `recriuter` ON `recriuter`.`id_profile` = `profiles`.`idprofiles` LEFT JOIN (SELECT `id_profile`, `value` FROM `processes` LEFT JOIN `process_details` ON `process_details`.`id_process` = `processes`.`idprocesses` WHERE `processes`.`name` = 'Second Interview' AND `process_details`.`name` = 'Second Interview') AS `ops` ON `ops`.`id_profile` = `profiles`.`idprofiles` LEFT JOIN (SELECT `company`, `id_profile` FROM `job_histories` LIMIT 1) AS `exp` ON `exp`.`id_profile` = `profiles`.`idprofiles` LEFT JOIN `accounts` ON `accounts`.`idaccounts` = `employees`.`id_account` LEFT JOIN (SELECT * FROM `users`) AS `usrs` ON `usrs`.`idUser` = `employees`.`reporter` LEFT JOIN `education_details` ON `education_details`.`id_profile` = `profiles`.`idprofiles` WHERE `date` BETWEEN $date;";

$output = fopen("php://output", "w");
fputcsv($output, array("No.", "Name", "Gender", "Birth Date", "Nearsol ID", "Hire Date", "Position", "Term. Motive", "Term. Kind", "Term. Reason", "Rehireable", "Term. Date", "Recriutment", "Operations", "Account", "Supervisor", "Last Job", "Educational Level", "Nearsol Experience", "Supervisor Experience"));
