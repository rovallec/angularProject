<?php
    ...
    if($result = mysqli_query($con, $sql)){
        while($row = mysqli_fetch_assoc($result)){
            ...
            if($g_2 == 'Single'){
                $gender = $gender . "Soltero(a), ";
            }else{
                $gender = $gender . "Casado(a), ";
            };

            $gender = $gender . $row['nationality'] . "(a)";

            $address = explode(",", $row['address'])[0] . ", " . explode(",", $row['address'])[1];
            $municipio = explode(",", $row['address'])[2];
            ...
            $incentivo_n = $incentivo_exp[0] . "." . str_pad($incentivo_exp[1], 2, "0");
            $total_n = $base_n + $incentivo_n + 250;
            ...
            $prs =
            $date_letters = $f->format($dt[0]) . " de " . $mnth[intval($dt[1])] . " de " . $f->format($dt[2]);

            $today_date = date("Y-m-d");
            $dt_end = explode("-", $today_date);
            $day = $dt_end[2];
            $mn = $mnth[intval($dt_end[1])];
            $yr = $dt_end[0];

            $ot = explode(".", $others);
            $other_letters = $f->format($ot[0]) . " quetzales con " . $f->format($ot[1]) . " centavos";
