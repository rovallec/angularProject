...
if($result = mysqli_query($con, $sql))
{
	...
	$sql2 = "SELECT employees.idemployees, UPPER(profiles.first_name) AS `first_name`, UPPER(profiles.second_name) AS `second_name`, UPPER(profiles.first_lastname) AS `first_lastname`,
			...
			employees.hiring_date, terminations.valid_from, UPPER(terminations.motive) AS `motive`, CONCAT(UPPER(terminations.motive), ' / ',UPPER(terminations.reason)) AS `reason`, UPPER(terminations.access_card) AS `access_card`,
	...
	if($result = mysqli_query($con, $sql2)){
		...
		$mail->IsHTML(true);
		$mail->AddAddress('agent-terms@nearsol.gt', 'Agent Terms');
