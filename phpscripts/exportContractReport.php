...
$sql = "SELECT 
        ...
        LEFT JOIN (select * from profiles p2 INNER join contact_details cd1 on (p2.idprofiles = cd1.id_profile) order by cd1.idcontact_details desc limit 1) cd ON (p.idprofiles = cd.id_profile)
...
if($result = mysqli_query($con,$sql)){
  while($row = mysqli_fetch_assoc($result)){ 
    ...
    $exportRow[14] = $row['marital_status'];
